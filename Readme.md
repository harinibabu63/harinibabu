...
### ❤️ Heart Rate Monitoring PCB | *Altium Designer*
`March 2024`

A production-ready PCB for photoplethysmography-based heart rate detection — designed end-to-end in Altium Designer.

- Executed full PCB design cycle: schematic capture → custom symbols/footprints → layout → routing → 3D verification
- Designed analog front-end using **LDR + LM358 op-amp**, with voltage threshold comparator triggering LED pulse output per heartbeat
- Applied production-ready trace width selection and component placement discipline throughout layout

**Tools:** `Altium Designer` `LM358 Op-Amp` `SMD Components` `Analog Front-End Design`

---

### 🪖 Smart Helmet System for Accident Detection & Prevention | *AutoCAD*
`May 2021` · **Published Research Paper**

An embedded safety system using **PIC16F84a microcontroller** with multi-sensor integration for accident prevention and automated emergency response.

- Implemented pre-ride safety interlocks: IR-based helmet detection + alcohol sensing via gas sensor
- Built automatic accident response pipeline: vibration sensor → GPS coordinate capture → location transmitted to nearest hospital
- Designed custom PCB with power regulation and analog/digital signal path separation

**Sensors:** `IR Sensor` `Gas Sensor (Breath Analyzer)` `Vibration Sensor` `MEMS` `Load Sensor` `GPS`  
**MCU:** `PIC16F84a` · **Comms:** `ZigBee` `GSM`

---

### 🚗 Vehicle Management System | *International Conference Project*
`Oct 2018` · **Presented at International Conference (IOP Publishing — MSE Vol. 994)**

A multi-layer vehicle safety system on **Raspberry Pi** combining computer vision, biometric monitoring, and crash detection.

- Implemented **driver drowsiness detection** using OpenCV and Eye Aspect Ratio (EAR) tracking, triggering real-time alerts on prolonged eye closure
- Integrated **vibration-based accident detection** with automatic GPS location push to server via Wi-Fi
- Added **heartbeat monitoring** with motor speed reduction logic for driver distress scenarios
- Designed SMPS power stage stepping down 230V → 12V with full signal conditioning PCB

**Tools:** `Raspberry Pi` `PIC Microcontroller` `OpenCV` `Python` `GPS/Wi-Fi` `SMPS`
...
|--------|-------------|--------|
| 🎓 **MS in Electrical Engineering** | California State University, Fullerton | Fall 2022 – Fall 2024 |
| 🎓 **BE in Electronics & Communication Engineering** | Anna University, Chennai | Fall 2017 – Spring 2021 |
...
![Harini's GitHub Stats](https://github-readme-stats.vercel.app/api?username=harinibabu&show_icons=true&theme=tokyonight&hide_border=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=harinibabu&layout=compact&theme=tokyonight&hide_border=true)
...
*"Good hardware doesn't just work — it works the first time."*
